...
export default async function ArticleDetailPage({ params }: { params: Params }) {
  ...
  // Note: we used to call `await publishDueArticles()` here so a visitor
  // hitting the URL exactly at publish time would see the article instead
  // of a 404. That DB write-on-read pattern forced Next to bail out of
  // static rendering for the whole detail page. With the ISR `revalidate`
  // of 30s kicking in and admin / sitemap triggers flipping scheduled
  // articles, worst-case a visitor sees 404 for up to ~30s after the
  // scheduled publish time — acceptable for a ~10x TTFB win on the
  // common case.
  const article = await prisma.article.findUnique({
    where: { slug },
    include: {
