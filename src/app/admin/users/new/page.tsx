...
export default function NewUserPage() {
  ...
  function toggleSection(key: string) {
    setPermissions((prev) => {
      ...
      if (current.enabled) {
        return { ...prev, [key]: { enabled: false, canCreate: false, canEdit: false, canDelete: false, canPublish: false } };
      }
  ...
  function selectAllSections() {
    setPermissions(Object.fromEntries(SECTIONS.map((s) => [s.key, { enabled: true, canCreate: true, canEdit: true, canDelete: true, canPublish: true }])));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    else { toast('Kullanıcı oluşturuldu'); router.push('/admin/users'); }
  }

  return (
    <div className="max-w-2xl">
      <h1 className="text-xl font-bold text-zinc-900 mb-1">Add New User</h1>
      <p className="text-xs text-zinc-500 mb-6">Create a new user and assign section-based permissions</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })}
                className="w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-zinc-900 outline-none" required minLength={6} />
            </div>
            <div>
          ...
          <div className="bg-white p-5 rounded-xl shadow-sm border space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-zinc-500 mt-0.5">Select which sections this user can access and what they can do</p>
              </div>
              <button type="button" onClick={selectAllSections}
            ...
            <div className="flex gap-3 text-[9px] text-zinc-400 pt-2">
              <span><span className="inline-block w-2 h-2 bg-emerald-100 rounded mr-1" />Create</span>
              <span><span className="inline-block w-2 h-2 bg-emerald-100 rounded mr-1" />Edit</span>
              <span><span className="inline-block w-2 h-2 bg-red-100 rounded mr-1" />Delete</span>
              <span><span className="inline-block w-2 h-2 bg-violet-100 rounded mr-1" />Publish</span>
            </div>
          </div>
        )}
