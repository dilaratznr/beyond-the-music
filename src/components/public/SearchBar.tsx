...
export default function SearchBar({ locale }: { locale: string }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {/* Results dropdown */}
      {open && query.length >= 2 && (
        <div className="absolute top-full right-0 mt-2 w-80 md:w-96 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden z-50">
          {loading ? (
              ...
              {results.map((r, i) => (
                ...
                >
                  {r.image ? (
                    ...
                    <div className="w-10 h-10 rounded-lg bg-zinc-800 flex items-center justify-center text-zinc-600 text-sm flex-shrink-0">
                      {r.type === 'genre' ? '♫' : r.type === 'artist' ? '♪' : r.type === 'article' ? '✎' : '◉'}
                    </div>
                  )}
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-white truncate">{r.title}</p>
                    <p className="text-[10px] text-zinc-500">{TYPE_LABELS[r.type] || r.type} · {r.sub}</p>
                  </div>
                </Link>
              ))}
              <Link
